...
PYBIND11_MODULE(cppimviz, m) {
	...
	m.def("toggle_fullscreent", [&]() {
		if (nullptr == glfwGetWindowMonitor(viz.window)) { // Windowed
			...
			if (nullptr != viz.window) {
				glfwSetWindowMonitor(viz.window, nullptr,
									 0, 0,
									 viz.windowSizeBeforeFullscreen.x,
									 viz.windowSizeBeforeFullscreen.y,
									 0);
			}
		}
